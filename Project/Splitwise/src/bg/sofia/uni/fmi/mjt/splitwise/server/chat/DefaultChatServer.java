...
public class DefaultChatServer implements ChatServer {
    ...
    public DefaultChatServer(InetSocketAddress mainServerAddress, String code) {
    ...
    public void sendMessage(String message, User author, Map<Socket, Socket> socketUsers) {
        for (Socket receiver : clients) {
            Optional<Socket> receiverSocketUser =
                    socketUsers.keySet().stream().filter(v -> v.getLocalPort() == receiver.getPort()).findFirst();
    ...
    public synchronized void disconnectUser(Socket socket) throws IOException {
        Optional<Socket> socketToRemove = clients
                .stream()
                .filter(c -> c.getLocalPort() == socket.getPort() && c.getPort() == socket.getLocalPort()).findFirst();
